...
[Tags("InvoiceItemType")]
[Route("api/invoice-item-types")]
public sealed class InvoiceItemTypeController : BaseApiController
{
    private readonly IUnitOfWork _uow;
    private readonly IMapper _mapper;

    public InvoiceItemTypeController(IUnitOfWork uow, IMapper mapper)
    {
        _uow = uow;
        _mapper = mapper;
    }
    ...
    {
        var items = await _uow.Repository<InvoiceItemType>().GetAllAsync(ct);
    ...
    {
        var item = await _uow.Repository<InvoiceItemType>().GetByIdAsync(id, ct);
