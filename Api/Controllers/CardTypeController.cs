...
public sealed class CardTypeController : BaseApiController
{
    private readonly IUnitOfWork _uow;
    private readonly IMapper _mapper;

    public CardTypeController(IUnitOfWork uow, IMapper mapper)
    {
        _uow = uow;
        _mapper = mapper;
    }
    ...
    {
        var items = await _uow.Repository<CardType>().GetAllAsync(ct);
    ...
    {
        var item = await _uow.Repository<CardType>().GetByIdAsync(id, ct);
