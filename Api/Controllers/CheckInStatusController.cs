...
[Tags("CheckInStatus")]
[Route("api/checkin-statuses")]
public sealed class CheckInStatusController : BaseApiController
{
    private readonly IUnitOfWork _uow;
    private readonly IMapper _mapper;

    public CheckInStatusController(IUnitOfWork uow, IMapper mapper)
    {
        _uow = uow;
        _mapper = mapper;
    }
    ...
    {
        var items = await _uow.Repository<CheckInStatus>().GetAllAsync(ct);
    ...
    {
        var item = await _uow.Repository<CheckInStatus>().GetByIdAsync(id, ct);
    ...
    {
        var item = new CheckInStatus(req.Nombre);
        await _uow.Repository<CheckInStatus>().AddAsync(item, ct);
    ...
    public async Task<IActionResult> Update(int id, [FromBody] CreateLookupRequest req, CancellationToken ct)
    {
        ...
        await _uow.Repository<CheckInStatus>().UpdateAsync(item, ct);
        await _uow.SaveChangesAsync(ct);
        return NoContent();
    }
    ...
    public async Task<IActionResult> Delete(int id, CancellationToken ct)
    {
        var item = await _uow.Repository<CheckInStatus>().GetByIdAsync(id, ct);
        if (item is null) return NotFound();
        await _uow.Repository<CheckInStatus>().RemoveAsync(item, ct);
        await _uow.SaveChangesAsync(ct);
        return NoContent();
    }
}
