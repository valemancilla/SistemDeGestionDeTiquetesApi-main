...
public sealed class StaffRoleController : BaseApiController
{
    private readonly IUnitOfWork _uow;
    private readonly IMapper _mapper;

    public StaffRoleController(IUnitOfWork uow, IMapper mapper)
    {
        _uow = uow;
        _mapper = mapper;
    }
    ...
    {
        var items = await _uow.Repository<StaffRole>().GetAllAsync(ct);
    ...
    {
        var item = await _uow.Repository<StaffRole>().GetByIdAsync(id, ct);
    ...
    {
        var item = new StaffRole(req.Nombre);
        await _uow.Repository<StaffRole>().AddAsync(item, ct);
    ...
    public async Task<IActionResult> Update(int id, [FromBody] CreateLookupRequest req, CancellationToken ct)
    {
        ...
        await _uow.Repository<StaffRole>().UpdateAsync(item, ct);
        await _uow.SaveChangesAsync(ct);
        return NoContent();
    }
    ...
    public async Task<IActionResult> Delete(int id, CancellationToken ct)
    {
        var item = await _uow.Repository<StaffRole>().GetByIdAsync(id, ct);
        if (item is null) return NotFound();
        await _uow.Repository<StaffRole>().RemoveAsync(item, ct);
        await _uow.SaveChangesAsync(ct);
        return NoContent();
    }
}
