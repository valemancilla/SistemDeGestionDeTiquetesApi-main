...
public sealed class ContinentController : BaseApiController
{
    private readonly IUnitOfWork _uow;
    private readonly IMapper _mapper;

    public ContinentController(IUnitOfWork uow, IMapper mapper)
    {
        _uow = uow;
        _mapper = mapper;
    }
    ...
    {
        var items = await _uow.Repository<Continent>().GetAllAsync(ct);
    ...
    {
        var item = await _uow.Repository<Continent>().GetByIdAsync(id, ct);
