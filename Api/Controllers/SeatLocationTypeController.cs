...
[Tags("SeatLocationType")]
[Route("api/seat-location-types")]
public sealed class SeatLocationTypeController : BaseApiController
{
    private readonly IUnitOfWork _uow;
    private readonly IMapper _mapper;

    public SeatLocationTypeController(IUnitOfWork uow, IMapper mapper)
    {
        _uow = uow;
        _mapper = mapper;
    }
    ...
    {
        var items = await _uow.Repository<SeatLocationType>().GetAllAsync(ct);
    ...
    {
        var item = await _uow.Repository<SeatLocationType>().GetByIdAsync(id, ct);
