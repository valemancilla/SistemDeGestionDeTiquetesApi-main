...
[Tags("FlightRole")]
[Route("api/flight-roles")]
public sealed class FlightRoleController : BaseApiController
{
    private readonly IUnitOfWork _uow;
    private readonly IMapper _mapper;

    public FlightRoleController(IUnitOfWork uow, IMapper mapper)
    {
        _uow = uow;
        _mapper = mapper;
    }
    ...
    {
        var items = await _uow.Repository<FlightRole>().GetAllAsync(ct);
    ...
    {
        var item = await _uow.Repository<FlightRole>().GetByIdAsync(id, ct);
